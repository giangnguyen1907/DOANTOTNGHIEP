...
use Api\Students\Model\PsReceivableStudentModel;
...
class StudentController extends BaseController
{
	...
	public function newfeedStudent(RequestInterface $request, ResponseInterface $response, array $args)
	{
		...
		$return_data['_msg_text'] = $return_data['message'] = 'Bảng tin của học sinh';
		...
		if (!PsAuthentication::checkDevice($user, $device_id)) {
			...
			/* Bản tin học sinh bao gồm:
			Thông tin điểm danh đến
			Điểm danh về
			Hoạt động
			Album
			Tin tức
			...
			*/
			...
			if ($ps_student) {
				...
				$class_id = $ps_student->class_id;

				// 10 ngày trước
				$today = date('Y-m-d');
				$daybefor = date("Y-m-d", strtotime($today . " -10 day"));

				// Lấy thông tin điểm danh của học sinh trong vòng 10 ngày gần nhất
				...
				$ps_diarys = $this->db->table(CONST_TBL_PS_LOGTIMES . ' as D')
					...
					->whereRaw('DATE_FORMAT(D.login_at, "%Y%m%d") >=' . date("Ymd", strtotime($daybefor)) . ' AND DATE_FORMAT(D.login_at, "%Y%m%d") <=' . date("Ymd", strtotime($today)))
				...
				// Lấy hoạt động của 10 ngày gần nhất
				$actives = $this->db->table(TBL_PS_ACTIVE_STUDENT . ' as C')
					->select('C.id', 'C.ps_class_id', 'C.title', 'C.note', 'C.start_at', 'C.start_time', 'C.end_time')
					->where('C.ps_class_id', '=', $class_id)
					->whereRaw('DATE_FORMAT(C.start_at, "%Y%m%d") >=' . date("Ymd", strtotime($daybefor)) . ' AND DATE_FORMAT(C.start_at, "%Y%m%d") <=' . date("Ymd", strtotime($today)))->orderBy('C.start_at', 'desc')->get();

				// Album đã tạo 10 ngày gần đây - album theo lớp
				$album_item = $this->db->table(TBL_PS_ALBUM_ITEMS . ' as A')
					->leftJoin(TBL_PS_ALBUMS . ' as B', 'B.id', '=', 'A.album_id')
					->select('A.album_id', 'A.id', 'A.url_thumbnail', 'A.url_file', 'B.title', 'B.note', 'B.id', 'B.number_like', 'B.count_comment', 'B.number_img', 'B.url_album', 'B.created_at')
					->where('B.is_activated', '=', 1)
					->where('B.ps_class_id', '=', $class_id)
					->where('A.is_activated', '=', 1)
					->whereRaw('DATE_FORMAT(B.created_at, "%Y%m%d") >=' . date("Ymd", strtotime($daybefor)) . ' AND DATE_FORMAT(B.created_at, "%Y%m%d") <=' . date("Ymd", strtotime($today)))->orderBy('B.created_at', 'desc')->get();


				// Tin tức đã đăng 10 ngày gần đây
				$articles = $this->db->table(TBL_PS_CMS_ARTICLES)
					->select('id', 'title', 'note', 'file_name AS url_image', 'created_at AS create_date')
					->where('ps_customer_id', '=', $user->ps_customer_id)
					->where('is_publish', '=', STATUS_ACTIVE)
					->whereRaw('DATE_FORMAT(created_at, "%Y%m%d") >=' . date("Ymd", strtotime($daybefor)) . ' AND DATE_FORMAT(created_at, "%Y%m%d") <=' . date("Ymd", strtotime($today)))->orderBy('created_at', 'desc')->get();

				// Thực đơn đã đăng 10 ngày gần đây
				$foods = $this->db->table(TBL_PS_MENUS_IMPORTS . ' as MN')
					->join(TBL_PS_MEALS . ' as M', 'M.id', '=', 'MN.ps_meal_id')
					->select('MN.ps_meal_id', 'M.title', 'MN.date_at', 'MN.description as food_title', 'MN.file_image as food_image')
					->where('M.is_activated', STATUS_ACTIVE)
					->whereRaw('DATE_FORMAT(MN.date_at, "%Y%m%d") >=' . date("Ymd", strtotime($daybefor)) . ' AND DATE_FORMAT(MN.date_at, "%Y%m%d") <=' . date("Ymd", strtotime($today)))
					->where('MN.ps_customer_id', $ps_student->ps_customer_id)
					->whereRaw('(MN.ps_object_group_id IS NULL OR MN.ps_object_group_id =' . $ps_student->ps_obj_group_id . ') and (MN.ps_workplace_id IS NULL OR MN.ps_workplace_id =' . $ps_student->ps_workplace_id . ')')
					->orderBy('MN.date_at', 'DESC')->distinct()->get();

				// Nhận xét 10 ngày gần đây
				$listReview = $this->db->table(TBL_PS_REVIEW . ' as a')
					->join(TBL_PS_CATEGORY_REVIEW . ' as C', 'a.category_review_id', '=', 'C.id')
					->selectRaw('a.category_review_id,C.title as category_title,a.id, a.student_id,  a.date_at, a.note')
					->whereRaw('DATE_FORMAT(a.date_at, "%Y%m%d") >=' . date("Ymd", strtotime($daybefor)) . ' AND DATE_FORMAT(a.date_at, "%Y%m%d") <=' . date("Ymd", strtotime($today)))
					->where('a.status', '=', 1)
				...
				for ($i = 0; $i <= 10; $i++) {
					...
					$ngayhomtruoc = date("Y-m-d", strtotime($today . " -" . $i . " day"));

					// Thông tin điểm danh
					$array_den = $array_ve = array();
					foreach ($ps_diarys as $diemdanh) {

						if (date("Y-m-d", strtotime($diemdanh->login_at)) == $ngayhomtruoc) {

							$array_den['gioden'] = date("H:i:s", strtotime($diemdanh->login_at));
							$array_den['gvdon'] = $diemdanh->_name_teacher;
							$array_den['nguoidua'] = $diemdanh->_name_take;
						}

						if (date("Y-m-d", strtotime($diemdanh->login_at)) == $ngayhomtruoc) {

							$array_ve['giove'] = date("H:i:s", strtotime($diemdanh->login_at));
							$array_ve['gvtra'] = $diemdanh->_name_teacher_reveler;
							$array_ve['nguoidon'] = $diemdanh->_name_reveler;
						}
					}

					$array_newfeed[$ngayhomtruoc]['diemdanhden'] = $array_den;
					$array_newfeed[$ngayhomtruoc]['diemdanhve'] = $array_ve;

					// Hoạt động
					$array_hoatdong = array();
					foreach ($actives as $key => $hoatdong) {

						if (date("Y-m-d", strtotime($hoatdong->start_at)) == $ngayhomtruoc) {
							$array_hoatdong[$hoatdong->id]['thoigian'] = $hoatdong->start_time . '-' . $hoatdong->end_time;
							$array_hoatdong[$hoatdong->id]['tieude'] = $hoatdong->title;
							$array_hoatdong[$hoatdong->id]['noidung'] = $hoatdong->note;
						}
					}

					$array_newfeed[$ngayhomtruoc]['hoatdong'] = $array_hoatdong;

					// Album
					$array_album = array();
					foreach ($album_item as $key2 => $album) {
						...
						if (date("Y-m-d", strtotime($album->created_at)) == $ngayhomtruoc) {

							$array_album[$album->album_id]['tieude'] = $album->title;
							$array_album[$album->album_id]['ghichu'] = $album->note;
							$array_album[$album->album_id]['number_like'] = $album->number_like;
							$array_album[$album->album_id]['status_like'] = $status_like;
							$array_album[$album->album_id]['count_comment'] = $album->count_comment;
							$array_album[$album->album_id]['number_img'] = $album->number_img;
							$array_album[$album->album_id]['url_file'][$key2 + 1] = $album->url_file;
							$array_album[$album->album_id]['url_thumbnail'][$key2 + 1] = $album->url_thumbnail;
						}
					}

					$array_newfeed[$ngayhomtruoc]['album'] = $array_album;

					// Tin tức
					$array_tintuc = array();
					foreach ($articles as $key3 => $tintuc) {
						if (date("Y-m-d", strtotime($tintuc->create_date)) == $ngayhomtruoc) {

							$array_tintuc[$tintuc->id]['id'] = $tintuc->id;
							$array_tintuc[$tintuc->id]['tieude'] = $tintuc->title;
							$array_tintuc[$tintuc->id]['ghichu'] = $tintuc->note;
							$array_tintuc[$tintuc->id]['url_image'] = $tintuc->url_image;
							$array_tintuc[$tintuc->id]['ngaydang'] = $tintuc->create_date;
						}
					}

					$array_newfeed[$ngayhomtruoc]['tintuc'] = $array_tintuc;

					// Thực đơn
					$array_thucdon = array();
					foreach ($foods as $key4 => $thucdon) {
						if (date("Y-m-d", strtotime($thucdon->date_at)) == $ngayhomtruoc) {

							$array_thucdon[$thucdon->ps_meal_id]['buaan'] = $thucdon->title;
							$array_thucdon[$thucdon->ps_meal_id]['monan'] = $thucdon->food_title;
							$array_thucdon[$thucdon->ps_meal_id]['hinhanh'] =  PS_CONST_URL_SERVER . $thucdon->food_image;
						}
					}
					$array_newfeed[$ngayhomtruoc]['thucdon'] = $array_thucdon;

					// Nhận xét hàng ngày
					$array_nhanxet = array();
					foreach ($listReview as $key5 => $nhanxet) {

						if (date("Y-m-d", strtotime($nhanxet->date_at)) == $ngayhomtruoc) {

							$array_nhanxet[$nhanxet->id]['danhmuc'] = $nhanxet->category_title;
							$array_nhanxet[$nhanxet->id]['nhanxet'] = $nhanxet->note;
						}
					}

					$array_newfeed[$ngayhomtruoc]['nhanxet'] = $array_nhanxet;
				}
			}

			$return_data['_data'] = $array_newfeed;

			//return $array_newfeed;
	...
	public function informationStudent(RequestInterface $request, ResponseInterface $response, array $args)
	{
		...
		// get data from URI
		$student_id = $args['student_id'];
		$this->logger->err('Thanhpv: ID' . $student_id);
		$code_lang = $this->getUserLanguage($user);
			...
			if ($student_id > 0) {
				...
				if ($ps_student) {
					...
					if ($ps_student->avatar != '') {
						...
						$this->logger->err('Thanhpv: ' . $user_info->avatar_url);
						if (!PsFile::urlExists($user_info->avatar_url)) {
							if ($ps_student->sex == STATUS_ACTIVE) {
								$user_info->avatar_url = PS_CONST_API_URL_PATH_CACHE_IMAGE . 'view_boy_avatar_default.png';
							} else {
								$user_info->avatar_url = PS_CONST_API_URL_PATH_CACHE_IMAGE . 'view_girl2_avatar_default.png';
							}
						}
					} else {
						if ($ps_student->sex == STATUS_ACTIVE) {
	...
	//danh sách đợt khám
	public function growthExaminationStudent(RequestInterface $request, ResponseInterface $response, array $args)
	{
		$return_data = array(
			'_msg_code' => MSG_CODE_FALSE,
			'_data' => []
	...
	//Chỉ số sức khỏe
	public function growthStudent(RequestInterface $request, ResponseInterface $response, array $args)
	{
			...
			if ($student_id > 0) {
				...
				if ($user->user_type == USER_TYPE_RELATIVE) {
					...
					if ($ps_student) {
						...
						$_data['growth_date'] =  $ps_student_growths;
						$_growth_info = array();
						...
						if (count($ps_student_growths) > 0 && $check_height > 0 && $check_weight > 0) {
							...
							foreach ($ps_student_growths as $obj) {
								$_data['height'] = PsNumber::number_clean($ps_student_growths[0]->height);

								$_data['weight'] = PsNumber::number_clean($ps_student_growths[0]->weight);
								$_data['tooth'] = $ps_student_growths[0]->index_tooth;
								$_data['throat'] = $ps_student_growths[0]->index_throat;
								$_data['eye'] = $ps_student_growths[0]->index_eye;
								$_data['heart'] = $ps_student_growths[0]->index_heart;
								$_data['lung'] = $ps_student_growths[0]->index_lung;
								$_data['skin'] = $ps_student_growths[0]->index_skin;
								$_data['note'] = $ps_student_growths[0]->note;
					...
					if ($ps_student) {
						...
						foreach ($ps_student_growths as $obj) {
							...
							if ($obj->height > 0) {
								...
								$growth_info = array();
								$growth_info['date_at'] = PsDateTime::toDMY($obj->input_date_at);
								$growth_info['student_id'] = (int) $obj->student_id;
								$growth_info['_age'] = $obj->index_age . " " . $psI18n->__('Month');
					...
					if ($ps_student) {
						...
						foreach ($ps_student_growths as $obj) {
							if ($obj->height > 0) {
								...
								switch ($obj->index_weight) {
									case -2:
										$growth_info['_index_weight'] = $psI18n->__('Severe malnutrition');
										break;
									case -1:
										$growth_info['_index_weight'] = $psI18n->__('Moderate malnutrition');
										break;
									case 0:
										$growth_info['_index_weight'] = $psI18n->__('Normal weight');
										break;
									case 1:
										$growth_info['_index_weight'] = $psI18n->__('Weight is higher than age');
										break;
									default:
										$growth_info['_index_weight'] = $psI18n->__('Normal weight');
								}
	...
	public function diaryAttendanceMonthStudent(RequestInterface $request, ResponseInterface $response, array $args)
	{
		...
		try {
			...
			if ($ps_student) {
				...
				//return 'AAAAAA'.count($ps_diarys);

				$return_data['_data'] = $ps_diarys;
			...
			if ($student_id > 0) {
				...
				if ($user->user_type == USER_TYPE_RELATIVE) {
					...
					if ($ps_student) {
						...
						// Lay cau hinh cua co so (Mau thuc don)
						$ps_work_places = PsWorkPlacesModel::getColumnById($ps_student->ps_workplace_id, 'config_template_menus as config_template_menus');
						//return $ps_work_places;
						if ($user_info->class_id <= 0 && !isset($ps_work_places)) {
							...
							if ($ps_work_places->config_template_menus == 1) { // Mẫu thực đơn mới
								...
								foreach ($meals as $meal) {
									...
									$foods = $this->db->table(TBL_PS_MENUS_IMPORTS . ' as MN')
										->join(TBL_PS_MEALS . ' as M', 'M.id', '=', 'MN.ps_meal_id')
										->select('MN.description as food_title', 'MN.file_image as food_image')
										->where('M.is_activated', STATUS_ACTIVE)
									...
									$meal_op = new \stdClass();

									$meal_op->meal_title = $meal->meal_title;

									$meal_op->meal_note = $meal->meal_note;
									...
									foreach ($foods as $key => $food) {
										...
										$food_op = new \stdClass();

										$food_op->title = PsString::htmlSpecialChars($food->food_title);
										$food_op->image = $food->food_image == "" ? "" : "https://quanly.nvoting.com/" . $food->food_image;

										$meal_op->food_op->$key = $food_op;
										...
										if ($food->food_image != '') {
											...
											$food_op->image = PsString::getUrlFoodImage($food->food_image);

											$web_content .= '<img src="' . PsString::getUrlFoodImage($food->food_image) . '" style="width:40%"/>';
										} elseif ($food->file_name != '') {
											$food_op->image = PsString::getUrlFoodImage($food->file_name);
											$web_content .= '<img src="' . PsString::getUrlPsImage($food->file_name) . '" style="width:40%"/>';
										}

										$web_content .= '</div>';

										// Title
										$web_content .= '<div class="w3-col s9" style="padding-left:5px;width: 100%">';

										$web_content .= '<div class="" style="padding-top:7px;white-space: pre-line;width: 100%">' . PsString::htmlSpecialChars($food->food_title) . '</div>';
									...
									$web_content .= '</tr>';
									$return_data['_data']['meal'][] = $meal_op;
								}
								...
								//return "AAAAAAAA".$web_content;

							} /*else { // Mẫu thực đơn cũ
								...
								foreach ($meals as $meal) {
									...
									foreach ($foods as $key => $food) {
										...
										if (PsString::trimString($food->note) != '') {
											$food_op->note = PsString::trimString($food->mn_note);
					...
					if ($ps_student) {
						...
						if ($user_info->class_id <= 0 || !$ps_work_places) {
							...
							if ($ps_work_places->is_receipt == M_FEE_NOTIFICATION) {
								...
								// return $ps_work_places->config_choose_charge_fee_mobile;
								...
								//chi tiết phiếu báo
								// return $user_info->date_info;
								$ps_ReceivableStudent = PsReceivableStudentModel::findOfStudentByDate($user_info->student_id, '01-' . $user_info->date_info);
								$a = array();
								$total_money = 0;
								if ($ps_receipt && $ps_receipt->is_public == STATUS_ACTIVE) {
									if ($ps_ReceivableStudent) {
										foreach ($ps_ReceivableStudent as  $ps_ReceivableStudents) {
											$total_money = $total_money +  $ps_ReceivableStudents->unit_price;
											//return $ps_ReceivableStudents;
											array_push($a, $ps_ReceivableStudents);
										}
										$return_data['_data']['fee'] = $a;
									} else {
										return ' no data ';
									}
								}
								...
								if ($ps_fee_reports && $ps_receipt && $ps_receipt->is_public == STATUS_ACTIVE) {
									// Lấy chi tiết các khoản thu -- Chưa có API
									if ($ps_work_places->config_choose_charge_fee_mobile == 0) {
										...
										$data_infos['dukien'] = (string) PsNumber::format_price($total_money, '');
										$data_infos['duthangtruoc'] = (string) PsNumber::format_price($ps_receipt->balance_last_month_amount, '');
										$data_infos['hoantra'] = (string) PsNumber::format_price($total_money - $ps_fee_reports->amount, '');
										$data_infos['tongnop'] = (string) PsNumber::format_price($ps_fee_reports->amount, '');
										$data_infos['danop'] = (string) PsNumber::format_price($ps_receipt->collected_amount, '');
										array_push($data, $data_infos);
									}
						...
						$return_data['_data']['user_info'] = $user_info;
						$return_data['_data']['data_infos'] = $data_infos;
						$return_data['_data']['content'] = $web_content;
	...
	// Hoat dong moi
	public function ClassActive(RequestInterface $request, ResponseInterface $response, array $args)
	{
		...
		try {
			...
			if (($student_id > 0) && ($user->user_type == USER_TYPE_RELATIVE)) {
				...
				if ($ps_student && $ps_student->class_id > 0) {
					...
					// Lay thong tin hoạt động
					$actives = $this->db->table(TBL_PS_ACTIVE_STUDENT . ' as C')
						->select('C.id', 'C.ps_class_id', 'C.title', 'C.note', 'C.start_time', 'C.end_time')
						->where('C.ps_class_id', '=', $ps_student->class_id)
						->where('C.start_at', '=', $date_at)->get();
					// ->where('C.is_activated', '=', STATUS_ACTIVE)
					// ->where('CR.is_global', '=', CAMERA_NOT_GLOBAL)->get();
				}
			}
			$data_info = array();
			...
			$return_data['_data']['data_info'] = $actives;
		} catch (Exception $e) {
			...
			if ($student_id > 0) {
				...
				if ($user->user_type == USER_TYPE_RELATIVE) {
					...
					if ($ps_student) {
						...
						if ($user->ps_customer_id > 0) {
							// -- BEGIN: Nhan xet thang, tuần
							$ps_comment_months = $this->db->table(TBL_PS_COMMENT_WEEK . ' as cw')
								->select('cw.title', 'cw.comment', 'cw.ps_month', 'cw.ps_week', 'cw.ps_year', 'cw.ps_customer_id', 'cw.created_at')
							...
							if ($ps_comment_months->ps_week > 0) {

								$web_content .= '<div class="w3-row ks-padding-7"><h5 class="w3-text-' . $app_config_color . '">' . $psI18n->__('Nhận xét tuần ') . $ps_comment_months->ps_week . '</h5></div>';
								...
								*/
								// Title + content
								$web_content .= '<div class="w3-col s12 w3-container">';
								$web_content .= '<div class="w3-text-' . $app_config_color . '">' . $ps_comment_months->comment . '</div>';
								$web_content .= '</div>';

								$web_content .= '</div><hr>';
							}

							if ($ps_comment_months->ps_month > 0) {
