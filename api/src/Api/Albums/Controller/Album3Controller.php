...
//use Api\PsMembers\Model\PsTeacherClassModel;
...
class AlbumController extends BaseController
{
	...
	// Lay danh sach album - của giáo viên / Phụ huynh
	public function listAlbums(RequestInterface $request, ResponseInterface $response, array $args)
	{
		...
		if ($user->user_type == USER_TYPE_TEACHER) {
			...
			// Danh sách album của giáo viên | Những album của lớp, Album do chính mình tạo
			$list_albums = AlbumModel::getListAlbumsOfMember($user->ps_customer_id,$user->id, $user->member_id);
			
			//return "AAAAAAAA";
			//$user->member_id
			
			// Danh sách album của giáo viên
			//$list_albums = AlbumModel::getListAlbumsOfMember($user->ps_customer_id,$user->member_id);
			...
			*/
			...
			$return_data = array(
				...
				'_msg_text' => (count($list_albums) <= 0) ? $psI18n->__('No albums available.') : '',
