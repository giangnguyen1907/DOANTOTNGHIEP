<?php

/**
 * BasePsConfigLatePayment
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $school_year_id
 * @property integer $ps_customer_id
 * @property integer $ps_workplace_id
 * @property date $from_date
 * @property date $to_date
 * @property double $price
 * @property string $note
 * @property boolean $is_activated
 * @property integer $user_created_id
 * @property integer $user_updated_id
 * @property PsSchoolYear $PsSchoolYear
 * @property PsCustomer $PsCustomer
 * @property PsWorkPlaces $PsWorkPlaces
 * @property sfGuardUser $UserCreated
 * @property sfGuardUser $UserUpdated
 * 
 * @method integer             getSchoolYearId()    Returns the current record's "school_year_id" value
 * @method integer             getPsCustomerId()    Returns the current record's "ps_customer_id" value
 * @method integer             getPsWorkplaceId()   Returns the current record's "ps_workplace_id" value
 * @method date                getFromDate()        Returns the current record's "from_date" value
 * @method date                getToDate()          Returns the current record's "to_date" value
 * @method double              getPrice()           Returns the current record's "price" value
 * @method string              getNote()            Returns the current record's "note" value
 * @method boolean             getIsActivated()     Returns the current record's "is_activated" value
 * @method integer             getUserCreatedId()   Returns the current record's "user_created_id" value
 * @method integer             getUserUpdatedId()   Returns the current record's "user_updated_id" value
 * @method PsSchoolYear        getPsSchoolYear()    Returns the current record's "PsSchoolYear" value
 * @method PsCustomer          getPsCustomer()      Returns the current record's "PsCustomer" value
 * @method PsWorkPlaces        getPsWorkPlaces()    Returns the current record's "PsWorkPlaces" value
 * @method sfGuardUser         getUserCreated()     Returns the current record's "UserCreated" value
 * @method sfGuardUser         getUserUpdated()     Returns the current record's "UserUpdated" value
 * @method PsConfigLatePayment setSchoolYearId()    Sets the current record's "school_year_id" value
 * @method PsConfigLatePayment setPsCustomerId()    Sets the current record's "ps_customer_id" value
 * @method PsConfigLatePayment setPsWorkplaceId()   Sets the current record's "ps_workplace_id" value
 * @method PsConfigLatePayment setFromDate()        Sets the current record's "from_date" value
 * @method PsConfigLatePayment setToDate()          Sets the current record's "to_date" value
 * @method PsConfigLatePayment setPrice()           Sets the current record's "price" value
 * @method PsConfigLatePayment setNote()            Sets the current record's "note" value
 * @method PsConfigLatePayment setIsActivated()     Sets the current record's "is_activated" value
 * @method PsConfigLatePayment setUserCreatedId()   Sets the current record's "user_created_id" value
 * @method PsConfigLatePayment setUserUpdatedId()   Sets the current record's "user_updated_id" value
 * @method PsConfigLatePayment setPsSchoolYear()    Sets the current record's "PsSchoolYear" value
 * @method PsConfigLatePayment setPsCustomer()      Sets the current record's "PsCustomer" value
 * @method PsConfigLatePayment setPsWorkPlaces()    Sets the current record's "PsWorkPlaces" value
 * @method PsConfigLatePayment setUserCreated()     Sets the current record's "UserCreated" value
 * @method PsConfigLatePayment setUserUpdated()     Sets the current record's "UserUpdated" value
 * 
 * @package    KidsSchool.vn
 * @subpackage model
 * @author     KidsSchool.vn <contact@kidsschool.vn>
 * @version    SVN: $Id: Builder.php 7490 2010-03-29 19:53:27Z jwage $
 */
abstract class BasePsConfigLatePayment extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('ps_config_late_payment');
        $this->hasColumn('school_year_id', 'integer', 11, array(
             'type' => 'integer',
             'notnull' => true,
             'length' => 11,
             ));
        $this->hasColumn('ps_customer_id', 'integer', null, array(
             'type' => 'integer',
             'unique' => false,
             'notnull' => true,
             ));
        $this->hasColumn('ps_workplace_id', 'integer', null, array(
             'type' => 'integer',
             'notnull' => true,
             ));
        $this->hasColumn('from_date', 'date', 10, array(
             'type' => 'date',
             'notnull' => false,
             'length' => 10,
             ));
        $this->hasColumn('to_date', 'date', 10, array(
             'type' => 'date',
             'notnull' => false,
             'length' => 10,
             ));
        $this->hasColumn('price', 'double', null, array(
             'type' => 'double',
             ));
        $this->hasColumn('note', 'string', 255, array(
             'type' => 'string',
             'length' => 255,
             ));
        $this->hasColumn('is_activated', 'boolean', null, array(
             'type' => 'boolean',
             'notnull' => true,
             'default' => true,
             ));
        $this->hasColumn('user_created_id', 'integer', null, array(
             'type' => 'integer',
             ));
        $this->hasColumn('user_updated_id', 'integer', null, array(
             'type' => 'integer',
             ));

        $this->option('type', 'InnoDB');
        $this->option('collate', 'utf8_unicode_ci');
        $this->option('charset', 'utf8');
    }

    public function setUp()
    {
        parent::setUp();
        $this->hasOne('PsSchoolYear', array(
             'local' => 'school_year_id',
             'foreign' => 'id'));

        $this->hasOne('PsCustomer', array(
             'local' => 'ps_customer_id',
             'foreign' => 'id',
             'onDelete' => 'CASCADE'));

        $this->hasOne('PsWorkPlaces', array(
             'local' => 'ps_workplace_id',
             'foreign' => 'id'));

        $this->hasOne('sfGuardUser as UserCreated', array(
             'local' => 'user_created_id',
             'foreign' => 'id'));

        $this->hasOne('sfGuardUser as UserUpdated', array(
             'local' => 'user_updated_id',
             'foreign' => 'id'));

        $timestampable0 = new Doctrine_Template_Timestampable();
        $this->actAs($timestampable0);
    }
}