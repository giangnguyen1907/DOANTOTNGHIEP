 ...
 */
class PsFeeReportsFormFilter extends BasePsFeeReportsFormFilter {
	...
	public function doBuildQuery(array $values) {
		...
		$query->addSelect ( 'fr.id AS id,fr.ps_fee_report_no AS ps_fee_report_no,fr.receivable AS receivable,fr.receivable_at AS receivable_at,fr.updated_at AS updated_at,sc.myclass_id AS class_id, re.id AS re_id,re.receipt_no AS receipt_no, re.receipt_date AS receipt_date, re.payment_status AS payment_status, re.collected_amount AS collected_amount,re.balance_amount AS balance_amount, re.payment_date AS payment_date, re.is_public AS is_public' );
		...
		// $query->leftJoin ( $a.'.Receipt re With (DATE_FORMAT(re.receipt_date,"%Y%m%d") = ?)', array(date ( 'Ymd', strtotime ( $receivable_at ) ) ) );
		...
		$query->orderBy ( 'fr.receivable_at DESC, s.last_name, s.first_name, mc.iorder' );
		// echo($query);die;
		return $query;
	}
}
