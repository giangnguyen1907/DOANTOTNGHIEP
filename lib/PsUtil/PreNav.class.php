	...
	public static function nav($page_nav) {
		...
		foreach ( $page_nav as $key => $nav_item ) {
			...
			if (isset ( $nav_item ["access"] ) && ($nav_item ["access"])) {
				...
				echo '<li ' . (isset ( $nav_item ["active"] ) && ($nav_item ["active"] == true) ? 'class = "active"' : '') . '>' . $nav_htm . '</li>';
			}
		}
		...
		if (isset ( $nav_item ["sub"] ) && $nav_item ["sub"]) {
			...
			foreach ( $nav_item as $key => $sub_item ) {
				...
				if (isset ( $sub_item ["access"] ) && ($sub_item ["access"])) {
					...
					$sub_item_htm_li .= '<li ' . (isset ( $sub_item ["active"] ) && ($sub_item ["active"] == true) ? 'class = "active"' : '') . '>
									<a href="' . $url . '" ' . $url_target . '>' . $icon . ' ' . $nav_title . $label_htm . '</a>
									' . (isset ( $sub_item ["sub"] ) ? self::sub_nav ( $sub_item ["sub"] ) : '') . '</li>';
				}
			}

			if ($sub_item_htm_li != '')
				$sub_item_htm = '<ul>' . $sub_item_htm_li . '</ul>';
		}
		return $sub_item_htm;
	}
}