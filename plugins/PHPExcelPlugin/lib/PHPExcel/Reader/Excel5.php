...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader {
	...
	private function readNote() {
		...
		if ($this->version == self::XLS_BIFF8) {
			...
			if ($extension) {
				...
				$this->phpSheet->getComment ( $cellAddress )
					->setText ( $this->parseRichText ( $noteText ) );
				// ->setAuthor($author)
			}
		}
	}
	...
	private function readXf() {
		...
		if (! $this->readDataOnly) {
			...
			if ($this->version == self::XLS_BIFF8) {
				...
				$objStyle->getBorders ()
					->getLeft ()->colorIndex = (0x007F0000 & self::getInt4d ( $recordData, 10 )) >> 16;

				// bit: 29-23; mask: 0x3F800000; right color
				$objStyle->getBorders ()
					->getRight ()->colorIndex = (0x3F800000 & self::getInt4d ( $recordData, 10 )) >> 23;
				...
				$objStyle->getBorders ()
					->getTop ()->colorIndex = (0x0000007F & self::getInt4d ( $recordData, 14 )) >> 0;

				// bit: 13-7; mask: 0x00003F80; bottom color
				$objStyle->getBorders ()
					->getBottom ()->colorIndex = (0x00003F80 & self::getInt4d ( $recordData, 14 )) >> 7;

				// bit: 20-14; mask: 0x001FC000; diagonal color
				$objStyle->getBorders ()
					->getDiagonal ()->colorIndex = (0x001FC000 & self::getInt4d ( $recordData, 14 )) >> 14;
				...
				switch ($extType) {
					case 4 : // fill start color
						...
						if ($xclfType == 2) {
							...
							if (isset ( $this->mapCellXfIndex [$ixfe] )) {
								$fill = $this->phpExcel->getCellXfByIndex ( $this->mapCellXfIndex [$ixfe] )
									->getFill ();
								$fill->getStartColor ()
									->setRGB ( $rgb );
					...
					case 13 : // font color
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if (isset ( $this->mapCellXfIndex [$ixfe] )) {
								$font = $this->phpExcel->getCellXfByIndex ( $this->mapCellXfIndex [$ixfe] )
									->getFont ();
								$font->getColor ()
									->setRGB ( $rgb );
	...
	private function readDefaultRowHeight() {
		...
		$this->phpSheet->getDefaultRowDimension ()
	...
	private function readLeftMargin() {
		...
		if (! $this->readDataOnly) {
			// offset: 0; size: 8
			$this->phpSheet->getPageMargins ()
				->setLeft ( self::extractNumber ( $recordData ) );
		}
	}
	...
	private function readRightMargin() {
		...
		if (! $this->readDataOnly) {
			// offset: 0; size: 8
			$this->phpSheet->getPageMargins ()
				->setRight ( self::extractNumber ( $recordData ) );
		}
	}
	...
	private function readTopMargin() {
		...
		if (! $this->readDataOnly) {
			// offset: 0; size: 8
			$this->phpSheet->getPageMargins ()
				->setTop ( self::extractNumber ( $recordData ) );
		}
	}
	...
	private function readBottomMargin() {
		...
		if (! $this->readDataOnly) {
			// offset: 0; size: 8
			$this->phpSheet->getPageMargins ()
				->setBottom ( self::extractNumber ( $recordData ) );
		}
	}
	...
	private function readPageSetup() {
		...
		if (! $this->readDataOnly) {
			...
			if (! $isNotInit) {
				...
				$this->phpSheet->getPageSetup ()
					->setFitToPage ( ( bool ) $this->isFitToPages );
				$this->phpSheet->getPageSetup ()
	...
	private function readDefColWidth() {
		...
		if ($width != 8) {
			$this->phpSheet->getDefaultColumnDimension ()
				->setWidth ( $width );
		}
	}
	...
	private function readColInfo() {
		...
		if (! $this->readDataOnly) {
			...
			for($i = $fc; $i <= $lc; ++ $i) {
				...
				$this->phpSheet->getColumnDimensionByColumn ( $i )
					->setWidth ( $width / 256 );
				$this->phpSheet->getColumnDimensionByColumn ( $i )
					->setVisible ( ! $isHidden );
				$this->phpSheet->getColumnDimensionByColumn ( $i )
				...
				$this->phpSheet->getColumnDimensionByColumn ( $i )
					->setXfIndex ( $this->mapCellXfIndex [$xfIndex] );
			}
		}
	}
		...
		if (! $this->readDataOnly && $this->readEmptyCells) {
			for($i = 0; $i < $length / 2 - 3; ++ $i) {
				...
				if (($this->getReadFilter () !== null) && $this->getReadFilter ()
					...
					$this->phpSheet->getCell ( $columnString . ($row + 1) )
						->setXfIndex ( $this->mapCellXfIndex [$xfIndex] );
				}
			}
		}
	...
	private function readDataValidation() {
		...
		foreach ( $cellRangeAddresses as $cellRange ) {
			...
			foreach ( PHPExcel_Cell::extractAllCellReferencesInRange ( $stRange ) as $coordinate ) {
				$objValidation = $this->phpSheet->getCell ( $coordinate )
					->getDataValidation ();
	...
	private function readContinue() {
		...
		$validSplitPoints = array (
				0xF003,
				0xF004,
				0xF00D ); // add identifiers if we find more
