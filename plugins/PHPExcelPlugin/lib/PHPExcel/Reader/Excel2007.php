...
class PHPExcel_Reader_Excel2007 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader {
												...
												$column->createRule ()
													->setRule ( 
														...
														null, array (
																...
																'second' => ( string ) $dateGroupItem ["second"] ), ( string ) $dateGroupItem ["dateTimeGrouping"] )
											...
											foreach ( $xmlDrawing->oneCellAnchor as $oneCellAnchor ) {
												if ($oneCellAnchor->pic->blipFill) {
													...
													if ($outerShdw) {
														...
														$shadow->getColor ()
															->setRGB ( self::getArrayItem ( $outerShdw->srgbClr->attributes (), "val" ) );
	...
	private static function readStyle($docStyle, $style) {
		...
		if (isset ( $style->border )) {
			...
			self::readBorder ( $docStyle->getBorders ()
	...
	private static function readBorder($docBorder, $eleBorder) {
		...
		if (isset ( $eleBorder->color )) {
			$docBorder->getColor ()
				->setARGB ( self::readColor ( $eleBorder->color ) );
		}
	}
				...
				foreach ( $is->r as $run ) {
					if (! isset ( $run->rPr )) {
						...
						if (isset ( $run->rPr->color )) {
							$objText->getFont ()
								->setColor ( new PHPExcel_Style_Color ( self::readColor ( $run->rPr->color ) ) );
						}
