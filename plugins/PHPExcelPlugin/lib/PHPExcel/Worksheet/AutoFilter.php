...
class PHPExcel_Worksheet_AutoFilter {
	...
	public function showHideRows() {
		...
		$this->workSheet->getRowDimension ( $rangeStart [1] )
			->setVisible ( true );
		...
		for($row = $rangeStart [1] + 1; $row <= $rangeEnd [1]; ++ $row) {
			...
			$this->workSheet->getRowDimension ( $row )
				->setVisible ( $result );
		}
