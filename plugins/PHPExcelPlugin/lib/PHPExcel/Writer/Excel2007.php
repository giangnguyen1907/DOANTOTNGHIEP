...
class PHPExcel_Writer_Excel2007 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter {
	...
	public function save($pFilename = null) {
		...
		if ($this->spreadSheet !== null) {
			...
			$saveDebugLog = PHPExcel_Calculation::getInstance ( $this->spreadSheet )->getDebugLog ()
				->getWriteDebugLog ();
			PHPExcel_Calculation::getInstance ( $this->spreadSheet )->getDebugLog ()
			...
			$customPropertiesPart = $this->getWriterPart ( 'DocProps' )
				->writeDocPropsCustom ( $this->spreadSheet );
			...
			for($i = 0; $i < $this->spreadSheet->getSheetCount (); ++ $i) {
				...
				if ($this->includeCharts) {
					$charts = $this->spreadSheet->getSheet ( $i )
						->getChartCollection ();
					if (count ( $charts ) > 0) {
			...
			for($i = 0; $i < $this->spreadSheet->getSheetCount (); ++ $i) {
				...
				$drawings = $this->spreadSheet->getSheet ( $i )
					->getDrawingCollection ();
				$drawingCount = count ( $drawings );
				if ($this->includeCharts) {
					$chartCount = $this->spreadSheet->getSheet ( $i )
						->getChartCount ();
				}
			...
			for($i = 0; $i < $this->getDrawingHashTable ()
				...
				} elseif ($this->getDrawingHashTable ()
