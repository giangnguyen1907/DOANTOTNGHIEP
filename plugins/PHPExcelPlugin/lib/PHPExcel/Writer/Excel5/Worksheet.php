...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter {
	...
	public function __construct(&$str_total, &$str_unique, &$str_table, &$colors, $parser, $preCalculateFormulas, $phpSheet) {
		...
		$this->countCellStyleXfs = count ( $phpSheet->getParent ()
	...
	public function close() {
		...
		// Column dimensions
		if (($defaultWidth = $phpSheet->getDefaultColumnDimension ()
			->getWidth ()) < 0) {
		...
		$autoFilterRange = $phpSheet->getAutoFilter ()
			->getRange ();
			...
			if ($cVal instanceof PHPExcel_RichText) {
				...
				foreach ( $elements as $element ) {
					...
					if ($element instanceof PHPExcel_RichText_Run) {
						$str_fontidx = $this->fontHashIndex [$element->getFont ()
							->getHashCode ()];
					} else {
						$str_fontidx = 0;
					}
	...
	private function writeWindow2() {
		...
		$fSelected = ($this->phpSheet === $this->phpSheet->getParent ()
			->getActiveSheet ()) ? 1 : 0;
		$fPaged = 1; // 2
		...
		$zoom_factor_page_break = ($fPageBreakPreview ? $this->phpSheet->getSheetView ()
			->getZoomScale () : 0x0000);
		$zoom_factor_normal = $this->phpSheet->getSheetView ()
			->getZoomScaleNormal ();
	...
	private function writeDefaultRowHeight() {
		...
		$defaultRowHeight = $this->phpSheet->getDefaultRowDimension ()
			->getRowHeight ();
	...
	private function writeSheetLayout() {
		...
		$this->colors [$this->phpSheet->getTabColor ()
			->getRGB ()], // color index
		0x0000 // unused
		);
	...
	private function writeSheetProtection() {
		...
		// prepare options
		$options = ( int ) ! $this->phpSheet->getProtection ()
			->getObjects () | ( int ) ! $this->phpSheet->getProtection ()
			->getScenarios () << 1 | ( int ) ! $this->phpSheet->getProtection ()
	...
	private function writeSetup() {
		...
		$iPaperSize = $this->phpSheet->getPageSetup ()
			->getPaperSize (); // Paper size

		$iScale = $this->phpSheet->getPageSetup ()
			->getScale () ? $this->phpSheet->getPageSetup ()
			->getScale () : 100; // Print scaling factor

		$iPageStart = 0x01; // Starting page number
		$iFitWidth = ( int ) $this->phpSheet->getPageSetup ()
			->getFitToWidth (); // Fit to number of pages wide
		$iFitHeight = ( int ) $this->phpSheet->getPageSetup ()
			->getFitToHeight (); // Fit to number of pages high
		$grbit = 0x00; // Option flags
		$iRes = 0x0258; // Print resolution
		$iVRes = 0x0258; // Vertical print resolution

		$numHdr = $this->phpSheet->getPageMargins ()
			->getHeader (); // Header Margin

		$numFtr = $this->phpSheet->getPageMargins ()
	...
	private function writeHcenter() {
		...
		$fHCenter = $this->phpSheet->getPageSetup ()
	...
	private function writeVcenter() {
		...
		$fVCenter = $this->phpSheet->getPageSetup ()
	...
	private function writeMarginLeft() {
		...
		$margin = $this->phpSheet->getPageMargins ()
	...
	private function writeAutoFilterInfo() {
		...
		$rangeBounds = PHPExcel_Cell::rangeBoundaries ( $this->phpSheet->getAutoFilter ()
			->getRange () );
	...
	private function writeProtect() {
		...
		if (! $this->phpSheet->getProtection ()
			->getSheet ()) {
			return;
		}
	...
	private function writePassword() {
		...
		if (! $this->phpSheet->getProtection ()
			->getSheet () || ! $this->phpSheet->getProtection ()
			->getPassword ()) {
			return;
		}
		...
		$wPassword = hexdec ( $this->phpSheet->getProtection ()
	...
	private function writeZoom() {
		...
		$header = pack ( "vv", $record, $length );
		$data = pack ( "vv", $this->phpSheet->getSheetView ()
			->getZoomScale (), 100 );
		$this->append ( $header . $data );
	}
	...
	private function writePageLayoutView() {
		...
		$wScalvePLV = $this->phpSheet->getSheetView ()
			->getZoomScale (); // 2
