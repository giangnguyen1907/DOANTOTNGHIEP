...
class PHPExcel_Writer_Excel5 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter {
	...
	private function writeDocumentSummaryInformation() {
		...
		if ($this->phpExcel->getProperties ()
			->getCategory ()) {
			$dataProp = $this->phpExcel->getProperties ()
				->getCategory ();
			$dataSection [] = array (
					'summary' => array (
							'pack' => 'V',
	...
	private function writeSummaryInformation() {
		...
		if ($this->phpExcel->getProperties ()
			...
			$dataSection [] = array (
					...
					'type' => array (
							'pack' => 'V',
							'data' => 0x1E ), // null-terminated string prepended by dword string length
					'data' => array (
							'data' => $dataProp,
							'length' => strlen ( $dataProp ) ) );
			$dataSection_NumProps ++;
		...
		if ($this->phpExcel->getProperties ()
			...
			$dataSection [] = array (
					...
					'type' => array (
							'pack' => 'V',
							'data' => 0x40 ), // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					'data' => array (
