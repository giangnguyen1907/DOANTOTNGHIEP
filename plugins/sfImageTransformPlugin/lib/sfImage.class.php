...
class sfImage {
	 ...
	 * @access public
	 * @param
	 *        	string Filename of the image to be loaded
	 * @param
	 *        	string File MIME type
	 * @param
	 *        	string Name of a supported adapter
	 */
	...
	/**
	 * Sets the adapter to be used, i.e.
	 * GD or ImageMagick
	 *
	 * @access public
	 * @param object $adapter
	 *        	Instance of adapter object to be used
	 */
	 ...
	 * @param
	 *        	string Backfground color of image
	 */
	public function create($x = null, $y = null, $color = null) {

		$defaults = sfConfig::get ( 'app_sfImageTransformPlugin_default_image', array (
	...
	public function copy() {
		...
		$copy = clone $this;
		$copy->setAdapter ( $this->getAdapter ()
			->copy () );
	...
	/**
	 * Magic method.
	 * This allows the calling of execute tranform methods on sfImageTranform objects.
	 *
	 * @method
	 * @param string $name
	 *        	the name of the transform, sfImage<NAME>
	 * @param
	 *        	array Arguments for the transform class execute method
	 * @return sfImage
	 */
	public function __call($name, $arguments) {

		$class_generic = 'sfImage' . ucfirst ( $name ) . 'Generic';
		$class_adapter = 'sfImage' . ucfirst ( $name ) . $this->getAdapter ()
			->getAdapterName ();
		...
		elseif (class_exists ( $class_generic, true )) {
			...
			throw new sfImageTransformException ( sprintf ( 'Unsupported transform %s. Cannot find %s adapter or generic transform class', $name, $this->getAdapter ()
				->getAdapterName () ) );
		}
	 ...
	 *
	 * @param
	 *        	integer Valid range is from 0 (worst) to 100 (best)
	 *        	
	 */
	...
	/**
	 * Returns mime type from the specified file type.
