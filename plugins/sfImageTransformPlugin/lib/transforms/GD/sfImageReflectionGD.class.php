...
class sfImageReflectionGD extends sfImageTransformAbstract {
	...
	public function transform(sfImage $image) {
		...
		$reflection->flip ()
			->resize ( $width, $this->reflection_height );

		$r_resource = $reflection->getAdapter ()
			->getHolder ();

		$dest_resource = $reflection->getAdapter ()
			->getTransparentImage ( $width, $height + $this->reflection_height );
