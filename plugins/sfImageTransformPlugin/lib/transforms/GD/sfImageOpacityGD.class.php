...
class sfImageOpacityGD extends sfImageTransformAbstract {
	 ...
	 *
	 * @param float $opacity
	 *        	If greater than 1, will be divided by 100
	 */
	 ...
	 *
	 * @param float $opacity
	 *        	Image between 0 and 1. If $opacity > 1, will be diveded by 100
	 * @return void
	 */
	...
	protected function transform(sfImage $image) {
		...
		$new_img = $image->getAdapter ()
			->getTransparentImage ( $image->getWidth (), $image->getHeight () );
		...
		for($x = 0; $x < $width; $x ++) {
			for($y = 0; $y < $height; $y ++) {
				$rgb = imagecolorat ( $image->getAdapter ()
					->getHolder (), $x, $y );
				$r = ($rgb >> 16) & 0xFF;
				$g = ($rgb >> 8) & 0xFF;
				$b = $rgb & 0xFF;
