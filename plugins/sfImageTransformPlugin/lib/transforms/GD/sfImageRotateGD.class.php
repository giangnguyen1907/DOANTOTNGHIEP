...
class sfImageRotateGD extends sfImageTransformAbstract {
	...
	protected function transform(sfImage $image) {
		...
		if ($this->background === '') {
			...
			$this->background = $image->getAdapter ()
				->getColorByHex ( $resource, $this->background );
		}

		// Easy
		if (function_exists ( "imagerotate" )) {
			$image->getAdapter ()
				->setHolder ( imagerotate ( $resource, $this->angle, $this->background ) );
		} // Manual
		// manual rotating based base on pilot at myupb dot com @ php.net
			...
			$dstImg = $image->getAdapter ()
				->getTransparentImage ( $dstw, $dsth );
