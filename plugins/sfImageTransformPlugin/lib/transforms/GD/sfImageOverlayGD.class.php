...
class sfImageOverlayGD extends sfImageTransformAbstract {
	...
	protected function transform(sfImage $image) {
		...
		$canvas_img = $image->getAdapter ()
			->getTransparentImage ( $canvas_w, $canvas_h );
		imagecopy ( $canvas_img, $resource, 0, 0, 0, 0, $canvas_w, $canvas_h );

		// Check we have a valid image resource
		if (false === $this->overlay->getAdapter ()
			->getHolder ()) {
		...
		$overlay_img = $this->overlay->getAdapter ()
			->getHolder ();
