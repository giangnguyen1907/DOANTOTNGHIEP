...
class sfImageRectangleGD extends sfImageTransformAbstract {
	 ...
	 *        	integer
	 * @param
	 *        	string/object hex or sfImage object
	 * @param
	 *        	integer
	 */
	...
	protected function transform(sfImage $image) {
		...
		if (! is_null ( $this->fill )) {
			if (! is_object ( $this->fill )) {
				imagefilledrectangle ( $resource, $this->x1, $this->y1, $this->x2, $this->y2, $image->getAdapter ()
					->getColorByHex ( $resource, $this->fill ) );
			}

			if ($this->getColor () !== "" && $this->fill !== $this->getColor ()) {
				imagerectangle ( $resource, $this->x1, $this->y1, $this->x2, $this->y2, $image->getAdapter ()
					->getColorByHex ( $resource, $this->getColor () ) );
			}
