...
class sfImageOverlayImageMagick extends sfImageTransformAbstract {
	 ...
	 *
	 * @param sfImage $overlay
	 *        	- the image for the overlay
	 * @param mixed $position
	 *        	- the named position as string, or the array of exact coordinates
	 * @param float $opacity
	 *        	- the opacity for the overlay image
	 * @param integer $compose
	 *        	- the composite operator
	 *        	
	 * @return void
	 */
	 ...
	 *        	named position. Possible named positions:
	...
	public function setPosition($position) {
		...
		if ($key = array_search ( $position, $map )) {
			...
			sfContext::getInstance ()->getEventDispatcher ()
				->notify ( new sfEvent ( $this, 'application.log', array (
					$message,
	 ...
	 *
	 * @param
	 *        	integer valid IMagick composite opeator
	 *        	
	 * @return void
	...
	protected function transform(sfImage $image) {
		...
		if (! is_null ( $this->getOpacity () )) {
			$overlay->getAdapter ()
				->getHolder ()
				->setImageOpacity ( $this->getOpacity () );
		}

		$resource->compositeImage ( $overlay->getAdapter ()
			->getHolder (), $this->getCompose (), $this->getLeft (), $this->getTop () );
