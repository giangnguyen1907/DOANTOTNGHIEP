...
class sfMailerExtended extends sfMailer {
	...
	protected $transports = array (), $curTransport = null, $maxRetries = 3, $retries = 0, $options = array (), $default_transport = null, $dispatcher = null;
	 ...
	 *        	transport name, null is for default transport defined in factory.yml
	 */
	...
	public function send(Swift_Mime_Message $message, &$failedRecipients = null, $transportName = null) {
		...
		try {
			...
			sfContext::getInstance ()->getLogger ()
				->info ( 'Error sending mail with transport ' . $transportName . ' attempt ' . $this->retries . ' of ' . $this->maxRetries );
			if ($this->retries < $this->maxRetries) {
				$fallBack = $this->transports [$this->curTransport] ['param'] ['fallback'];
				$ret = $this->send ( $message, $failedRecipients, $fallBack );
			} else {
				sfContext::getInstance ()->getLogger ()
					->info ( 'Error sending mail after ' . $this->retries . ' retries' );
				$this->retries = 0;
