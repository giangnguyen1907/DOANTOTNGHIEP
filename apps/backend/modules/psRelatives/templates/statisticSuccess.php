...
$username = 0;
	if ($ps_class_id > 0) {
		...
		$total_accounts = Doctrine::getTable ( 'sfGuardUser' )->countUserActivityAppByWorkplace ( $ps_customer_id, $ps_workplace_id );
		foreach ( $total_accounts as $relative ) {
		...
		<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			...
			<div class="jarviswidget" id="wid-id-0"
				...
				<header>
					...
					<h2><?php echo __('Statistic account relatives by relatives') ?></h2>
				</header>
    							...
    							<div class="text-center"
									...
									<table id="dt_basic"
										...
										foreach ( $class_list as $class ) {
											...
											$class_logging_now = 0;
											$class_username = 0;
											foreach ( $filter_member_statistic as $key => $relative ) {
												if ($class ['id'] == $relative->getMcId ()) {
										...
										?>
            							<tr>
											<td class="text-center"><?php echo "{$class['title']}<br><span><small class='text-muted'><i></i></small></span>" ?></td>
											<td class="text-center"><?php echo $class_count; ?></td>
											<td class="text-center"><?php echo $class_username; ?></td>
											<td class="text-center"><?php echo $class_active_app; ?></td>
											<td class="text-center"><?php echo $class_logging_now; ?></td>
										</tr>
            							<?php }?>
            							
            						</tbody>
									</table>
            					<?php echo '* ' . __('Sometime, the total data not equal with total data of all class because some relative can have many children studied') ?>
    							<?php endif;?>
...
<?php }else{
    include_partial('psRelatives/statistic_student', array('school_year_id' => $school_year_id,'ps_customer_id' => $ps_customer_id,'ps_workplace_id'=> $ps_workplace_id,'ps_class_id'=>$ps_class_id,'filter_member_statistic'=>$filter_member_statistic,'class_list'=>$class_list,'students'=>$students,'formFilter' => $formFilter, 'helper' => $helper));
}?>