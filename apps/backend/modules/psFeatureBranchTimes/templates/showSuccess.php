...
$(document).ready(function() {
	...
	// Xuất thơi khoa bieu
	$('.export_feature_branch').click(function() {
		
		if ($('#feature_branch_times_filters_ps_workplace_id').val() <= 0) {
		...
		$('#ps-filter').attr('action', '<?php echo url_for('@ps_feature_branch_times_export_by_class') ?>');
		$('#ps-filter').submit();
      ...
      <div class="jarviswidget" id="wid-id-0"
				...
				<header>
					...
					<div class="widget-toolbar">
						<a class="btn btn-default btn-success btn-sm btn-psadmin btn-delete hidden-xs export_feature_branch" id="export_feature_branch"
							href="javascript:;"><i class="fa-fw fa fa-cloud-download" title="<?php echo __('Export file') ?>"></i> <?php echo __('Export file') ?>
						</a>
						<?php if($sf_user->hasCredential('PS_SYSTEM_FEATURE_BRANCH_IMPORT')){ ?>
						<a target="_blank"
							...
							title="<?php echo __('Import file') ?>"></i> <?php echo __('Import template 1') ?></a>
						<a target="_blank"
							class="btn btn-default btn-success btn-sm btn-psadmin btn-delete hidden-xs"
							href="<?php echo url_for(@ps_feature_branch_times_import) ?>"><i
							class="fa-fw fa fa-cloud-upload"
							title="<?php echo __('Import file') ?>"></i> <?php echo __('Import template 2') ?></a>
						<a target="_blank"
							class="btn btn-default btn-success btn-sm btn-psadmin btn-delete hidden-xs"
							href="<?php echo url_for(@ps_feature_branch_times_import_tem3) ?>"><i
							class="fa-fw fa fa-cloud-upload"
							title="<?php echo __('Import file') ?>"></i> <?php echo __('Import template 3') ?></a>
						<?php } ?>
					</div>
				</header>
            ...
            <div class="widget-body no-padding">
						...
						<table id="tbl-List-User" class="table table-bordered table-striped" width="100%">
							...
							<thead>
								<tr class="header hidden-sm hidden-xs">
									...
									<div><?php echo __('Hour working')?></div></th>
            	...
            	<?php if(count($list_hour) > 0):?>
        		
        		<?php foreach ($list_hour as $hour):?>
        		<tr>
        			<td class="text-center"><?php echo format_date($hour->getStartTime(), "HH:mm") ?> &rarr; <?php echo format_date($hour->getEndTime(), "HH:mm") ?>  </td>
        			<?php foreach ($week_list as $date => $monday): ?> 
        			<td>
        			<?php foreach ($list_menu as $key => $fbtimes):?>
	        			...
	        			if (strtotime($date) >= strtotime($fbtimes->getStartAt ()) && strtotime($date) <= strtotime($fbtimes->getEndAt ()) && format_date($hour->getStartTime(), "HH:mm") == format_date($fbtimes->getStartTime(), "HH:mm")) {
        		...
        		<?php else:?>
        			<tr>
						<td colspan="8"><b><?php echo __('Not feature branch times by week')?></b>
						</td>
					</tr>
        		<?php endif;?>
        	</tbody>
