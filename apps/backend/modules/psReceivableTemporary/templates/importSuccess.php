...
<?php include_partial('psReceivableTemporary/assets')?>
<style>
.float-right {
	float: right;
	margin-right: 20px;
...
$(document).ready(function() {
	...
	$('#import_filter_ps_customer_id').change(function() {
		...
		if ($(this).val() > 0) {
			...
			$("#import_filter_ps_receivable_id").attr('disabled', 'disabled');
	    	...
	    	$.ajax({
		  	      ...
		  	      type: "POST",
		  	      data: 'y_id=' + $('#import_filter_ps_school_year_id').val() + '&psc_id=' + $('#import_filter_ps_customer_id').val(),
		  	      processResults: function (data, page) {
		  	          return {
		  	            results: data.items  
		  	          };
		  	    	  ...
		  	    	  $('#import_filter_ps_receivable_id').select2('val','');
		  	    	  $("#import_filter_ps_receivable_id").html(msg);
		  	    	  $("#import_filter_ps_receivable_id").attr('disabled', null);
		  	      });
			}		
		});
		...
		if ($(this).val() > 0) {
	    	...
	    	$.ajax({
		  	      url: '<?php echo url_for('@ps_receivable_student_by_customer') ?>',
		  	      type: "POST",
		  	      data: 'y_id=' + $('#import_filter_ps_school_year_id').val() + '&psc_id=' + $('#import_filter_ps_customer_id').val() + '&wp_id=' + $('#import_filter_ps_workplace_id').val(),
		  	      processResults: function (data, page) {
		  	          return {
		  	            results: data.items  
		  	          };
...
<section id="widget-grid">
	<div class="row">
		<?php include_partial('psReceivableTemporary/flashes2')?>
		...
		<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			<div class="jarviswidget" id="wid-id-0"
				...
				<header>
					...
					<h2><?php echo __('Receivable student import', array(), 'messages') ?></h2>
					<?php $path_file = '/media-template/import/khoanphaithu.xlsx'; ?>
					
        			<div class="widget-toolbar">
						<a class="btn btn-xs btn-success" href="<?php echo $path_file ?>"><i
				...
				<div id="sf_admin_content">
					...
					<div class="sf_admin_form widget-body">
					    ...
					    <?php endif; ?>
							...
							action="<?php echo url_for('@ps_receivable_temporary_import_save') ?>"
							...
							<fieldset>
								...
								<div class='col-md-6'>
									<div class="sf_admin_form_row sf_admin_foreignkey"
										style="margin-top: 25px">
										<label class="col-md-3 control-label">
                        		 		<?php echo __('Select receivable', array(), 'messages') ?>
                        		  	</label>
										<div class="col-md-9">
                            		 	<?php echo $formFilter['ps_receivable_id']->render() ?>
                            		 	<?php echo $formFilter['ps_receivable_id']->renderError() ?>
                            		 </div>
									</div>
								</div>
							...
							<div class="form-actions">
								<div class="sf_admin_actions">
									<a
										...
										href="<?php echo url_for('@ps_receivable_temporary') ?>"><i
