...
<table id="dt_basic"
	...
	<thead>
		<tr>
          <?php include_partial('psCmsNotifications/list_th_tabular', array('sort' => $sort)) ?>
	...
	<tbody>
        <?php foreach ($pager->getResults() as $i => $ps_cms_notifications): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          <tr class="sf_admin_row <?php echo $odd ?>">
			            ...
			            <?php include_partial('psCmsNotifications/list_td_tabular', array('ps_cms_notifications' => $ps_cms_notifications, 'filter_value' => $filter_value)) ?>
						            <?php include_partial('psCmsNotifications/list_td_actions', array('ps_cms_notifications' => $ps_cms_notifications, 'helper' => $helper, 'filter_value' => $filter_value)) ?>
			                        <?php include_partial('psCmsNotifications/list_td_batch_actions', array('ps_cms_notifications' => $ps_cms_notifications, 'helper' => $helper)) ?>
                </tr>
        <?php endforeach; ?>
      </tbody>
</table>
