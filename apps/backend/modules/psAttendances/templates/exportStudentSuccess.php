...
$('#export_filter_ps_school_year_id').change(function() {
	...
	resetOptions('export_filter_class_id');
	$('#export_filter_class_id').select2('val','');
	resetOptions('export_filter_ps_month');
	$('#export_filter_ps_month').select2('val','');
	if ($('#ps_logtimes_filters_ps_customer_id').val() <= 0) {
		return;
	}

	$("#export_filter_ps_month").attr('disabled', 'disabled');
	...
	$.ajax({
        ...
        processResults: function (data, page) {
	    	...
	    	$('#export_filter_ps_month').select2('val','');
			$("#export_filter_ps_month").html(msg);
			$("#export_filter_ps_month").attr('disabled', null);
	    });
...
<section id="widget-grid">
		...
		<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			...
			<div class="jarviswidget" id="wid-id-0"
				...
				<header>
					...
					<h2><?php echo __('Attendances export', array(), 'messages') ?></h2>
    ...
    <?php endif; ?>
		<form id="ps-filter" class="form-inline pull-left"
								action="<?php echo url_for('ps_attendances_collection', array('action' => 'exportStudent')) ?>"
								method="post">
								<div class="pull-left">
    	 	...
    	 	<div class="form-group">
					...
					<b style="color: #f00"><?php echo __('Note').': '?></b><strong><?php echo __('Export data student in class by month')?></strong>
				</div>
			</div>
