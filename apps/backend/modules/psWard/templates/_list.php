...
<table id="dt_basic"
	...
	<thead>
		<tr>
          <?php include_partial('psWard/list_th_tabular', array('sort' => $sort)) ?>
	...
	<tbody>
        <?php foreach ($pager->getResults() as $i => $ps_ward): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          <tr class="sf_admin_row <?php echo $odd ?>">
			
			            <?php include_partial('psWard/list_td_tabular', array('ps_ward' => $ps_ward)) ?>
						            <?php include_partial('psWard/list_td_actions', array('ps_ward' => $ps_ward, 'helper' => $helper)) ?>
			                        <?php include_partial('psWard/list_td_batch_actions', array('ps_ward' => $ps_ward, 'helper' => $helper)) ?>
                </tr>
        <?php endforeach; ?>
      </tbody>
</table>
