		...
		<!-- sf_admin_container -->
			<div class="jarviswidget" id="wid-id-0"
				...
				<header>
					...
					<h2><?php echo __('Classroom performance assessment', array(), 'messages') ?>				
			</h2>
				</header>
				<div>
						...
						<div id="datatable_fixed_column_wrapper"
							...
							<div class="dt-toolbar no-margin no-padding no-border">
								<div class="col-xs-12 col-sm-12">
									<div id="sf_admin_header"><?php include_partial('psStudentServiceCourseComment/list_header', array('pager' => $pager)) ?></div>
								</div>
							</div>
					...
					<form id="frm_batch"
								action="<?php echo url_for('ps_student_service_course_comment_save', array('action' => 'batch')) ?>"
								method="post">
								<input type="hidden" name="tracked_at"
									value="<?php echo $filter_value['tracked_at'] ;?>" /> <input
									type="hidden" name="ps_service_course_schedule_id"
									value="<?php echo $filter_value['ps_service_course_schedule_id'] ;?>" />
					<?php include_partial('psStudentServiceCourseComment/list', array('pager' => $pager, 'sort' => $sort, 'helper' => $helper, 'filter_value' => $filter_value)) ?>
					...
					<!-- sf_admin_footer -->
				    	...
				    	<?php if ($pager->haveToPaginate()): ?>
      					<?php include_partial('psStudentServiceCourseComment/pagination', array('pager' => $pager)) ?>
    					<?php endif; ?>
				    	</div>

									<div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 text-right">
				    	<?php if($check_current_date): ?>					      
					      <?php include_partial('psStudentServiceCourseComment/list_batch_actions', array('helper' => $helper)) ?>	
					      <?php //include_partial('psStudentServiceCourseComment/list_actions', array('helper' => $helper)) ?>
					    <?php endif; ?>
				      </div>
								</div>
								<!-- END: sf_admin_footer -->
