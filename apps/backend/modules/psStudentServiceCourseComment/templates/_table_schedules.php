...
<?php
// $edit_ps_service_course_schedule_id = (isset($ps_service_course_schedules) && $ps_service_course_schedules->getId() > 0) ? $ps_service_course_schedules->getId() : null;
?>
	...
	<table id="tbl-List-User" class="table table-bordered table-striped">
		...
		<thead>
			<tr>
				...
				<th class="text-center <?php if (date('N', strtotime($date)) == 6) echo 'bg-color-yellow'; elseif (date('N', strtotime($date)) == 7) echo 'bg-color-pink';?>" style="width: <?php echo $width_th?>%;">
					<b><?php echo __($monday)?><br>
						<div class="date"><?php echo format_date($date, "dd-MM-yyyy");?></div>
				</b>
				</th>
				<?php endforeach;?>
			</tr>
					...
					<ul class="media-list" style="list-style-type: none;">
						...
						<?php if (date('Y-m-d', strtotime($course_schedules->getDateAt())) == $date &&  ($session))  :?>
							...
							<div class="media-body">
								<ul class="media-list" style="list-style-type: none;">
									<li><b><?php echo $course_schedules->getCoursesTitle()?></b></li>
									<li>
