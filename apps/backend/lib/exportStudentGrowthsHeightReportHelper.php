<?php
class exportStudentGrowthsHeightReportHelper extends ExportHelper {
	...
	public function setStatisticInfoExportGrowths($class_name, $examination) {
		...
		$this->objPHPExcel->getActiveSheet ()
			->setCellValue ( 'B5', $class . ' : ' . $class_name->getMcName () );
		$this->objPHPExcel->getActiveSheet ()
			->setCellValue ( 'D5', $examina . ' : ' . $examination->getName () . ' , ' . $date . ' : ' . $inputdateat );
	}
	...
	public function setDataExportStatistic($growths_weight, $examination) {
		...
		$this->objPHPExcel->getActiveSheet ()
			->setCellValue ( 'E' . $start_row, $day . ' ' . date ( 'd' ) . ' ' . $month . ' ' . date ( 'm' ) . ' ' . $year . ' ' . date ( 'Y' ) );
	}
}